...
#list of Geners learnt from the Training Data
genre_list = [ 'action', 'adult', 'adventure', 'animation', 'biography', 'comedy', 'crime', 'documentary', 'family', 'fantasy', 'game-show', 'history',]
...
try:
    with tqdm(total=50, desc="Loading Train Data") as pbar:
        train_data = pd.read_csv('train_data.txt', sep=':::', header=None, names=['SerialNumber', 'MOVIE_NAME', 'GENRE', 'MOVIE PLOT'], engine='python')        
        pbar.update(50)
except Exception as e:
    print("Error loading train_data: {e}")
    raise
...
tfidf_vectorizer = TfidfVectorizer(max_features=5000) # You can adjust max features
